...
image="${AGAVE_IMAGE:-solana-diff/agave:4.3.0-arm64}"
...
if (( $(free_kib) < (floor_gib + 7) * 1024 * 1024 )); then
  echo "Need at least $((floor_gib + 7)) GiB free to attempt this build; 20+ GiB is recommended." >&2
  exit 1
fi
...
docker buildx build "${build_args[@]}" \
  --build-context "agave=$source_dir" --build-arg "BUILD_JOBS=$jobs" \
  --file Dockerfile.agave --tag "$image" . &
build_pid=$!
cancel() {
